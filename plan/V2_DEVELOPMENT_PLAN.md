...
- ~~`gos-hal::display` trait：`init(prefer: ResolutionHint) -> Surface` / `surface.lfb()` / `surface.flip()`。Bochs-VBE 为 backend #1（封装当前 `fbtest.rs:296` 的 DISPI 直写），为 virtio-gpu / UEFI GOP 留口。~~ **[ADR-007 已选向，2026-08-03]**：该草案引用的 `fbtest.rs:296` 在 V2 代码库中从未存在；`k-vk-host`（host-bridged graph-native `@gos.vk` 显示列表）已实质性满足本阶段的图形交付需求。`gos-hal::display`（真正的 LFB `Surface` trait）范围收窄至 **V2.6"真机显示"**（UEFI GOP / virtio-gpu backend，见 [ADR-013](../doc/ADR-013-real-hardware-display-mvp.md)），与 `k-vk-host` 并列、非替代关系，不在 V2.4 阻塞范围内。详见 [ADR-007](../doc/ADR-007-display-hal-scope.md)。
