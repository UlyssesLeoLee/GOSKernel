...
### E.1 Tick-driven preemption
- PIT 中断调用 `gos_supervisor::on_tick()`
- per-instance 时间片预算（默认 10 ms）
- 用尽 → instance `Yielding`、re-enqueue ready 队列尾
- 完成判据：死循环 plugin 不卡死系统

### E.2 Ring 3 切换基础
- TSS / GDT 加 user code/data
- `IA32_LSTAR` / `IA32_FMASK` / `IA32_STAR` 配置
- syscall handler：user 调 supervisor ABI（alloc_pages / emit_signal …）走 syscall 入口
- 完成判据：测试 plugin 在 Ring 3 跑通 alloc + emit_signal

### E.3 Native plugin 迁到 Ring 3
- manifest 新增 `privilege: Privilege::Kernel | Privilege::User`
- supervisor 据此决定 trampoline 是否切 Ring
- 把 1–2 个 leaf plugin（k-cypher、k-ai）切 User
- 强制访问 0 地址 #PF → 走 B.5 degrade

### E.4 SMP 启动（可推迟）
- LAPIC + AP 唤醒 + per-CPU 数据 + TLB shootdown
- `RUNTIME` 锁分片以支持多实例并发 dispatch
...
### F.1 Block 驱动
- AHCI（QEMU 默认）；NVMe follow-up
- `BlockDevice` capability：`read_sector / write_sector`

### F.2 VFS 抽象
- `gos-vfs` crate：`Inode / Dirent / FileSystem` trait
- `RESOURCE_FILE_HANDLE` 进 supervisor 配额

### F.3 FAT32 read-only
- minimal FAT32 read（可选 `fatfs`）
- shell `ls / cat`
