...
### F.3 FAT32 read-only
- F.3.1 ✅ 已完成（2026-04-26）—— 新 crate `k-fat32`，自研 no_std FAT32 reader
  - BPB 解析 + 严格校验（512-byte sector、power-of-two cluster、FAT32-only）
  - 实现 `gos_vfs::FileSystem`：`lookup` / `read` / `read_dir`
  - 8.3 短名解析（LFN slot 跳过）；attribute 解码 file/directory
  - 单 cluster 读（FAT chain walking 是 F.3.2）
  - 回归：手工拼装 32 KiB FAT32 image → 端到端 BPB / readdir / lookup / read（含 offset）/ NotFound 错误
- F.3.2 后续：FAT chain walking（>1-cluster 文件）
- F.3.3 后续：subdirectory traversal、LFN
- shell `ls / cat` 命令绑定（独立切片）

### F.4 Graph state 持久化 ✅ 已完成（2026-04-27 — serializer + replay）
- 新 crate `gos-journal`（no_std）
- 固定 40 字节 on-disk envelope 格式（独立于 `ControlPlaneEnvelope` 内存布局）
- `JournalHeader { magic="GOSJ", version=1, record_size=40 }` 8 字节文件头
- `JournalRing<N>`：bounded append buffer，`flush_into(out)` 把 header+records 序列化到调用方提供的 byte 缓冲
...
### G.3 socket + smoltcp（结构性 ✅ 已完成 2026-04-27）
- gos-protocol 新增 `socket` 子模块：`SocketDeviceVTable / SocketAddress / SocketOptions / SocketKind / SocketStatus`
- `RESOURCE_SOCKET` 注册到 supervisor 默认资源表
- 回归：`socket_vtable_round_trips_open_send_recv_close`（runtime harness）— stub vtable 跑 open→send→recv→close，每个 callback 命中计数 + 参数检查
- 后续切片：smoltcp 接入 + 替换 k-net 当前 ad-hoc ARP/ICMP

### G.4 GPU 资源化 ✅ 已完成（2026-04-27）
- `RESOURCE_GPU_MEMORY` 单独注册（`RESOURCE_GPU_ACCEL` 保留为 "want GPU compute" 语义）
- `InstanceRecord` 新增 `gpu_bytes_used / gpu_bytes_quota`
- supervisor public API：`set_gpu_quota / charge_gpu_bytes / credit_gpu_bytes / instance_gpu_usage`
- 不变量：默认 quota=0（即拒绝任何 GPU 分配）；degraded module 拒绝；超过 quota 返回 `HeapQuotaExceeded`
- 回归：`g3_g4_resources_registered_and_gpu_quota_enforced` — claim → quota=0 拒绝 → set 1 MiB → 512 KiB 通过 → 600 KiB 超额拒绝 → credit 256 KiB → 600 KiB 通过
...
| 子项 | 状态 | 落地内容 |
|---|---|---|
| H.1 Cypher 写支持 | ✅ | 新 crate `gos-cypher-mut`：`CypherMutation` enum（AddEdge/RemoveEdge/RebindUse 三种受控变体）、`ReceptiveEdgeKind`（Mount/Use 限定）、`MutationDispatcher` trait、`AuditedMutation` -> control-plane envelope |
| H.2 LLM-driven 控制面 | ✅ | 新 crate `gos-ai-bridge`：`LlmRequest/LlmResponse/AcceptanceMode`、`LlmBackend` hook、`MutationGate` 操作员审批队列；建议必须通过 H.1 `pre_validate` |
| H.3 分布式图 | ✅ | 新 crate `gos-cluster`：`HostId/RemoteVector/PeerRecord`、`announce_peer/forget_peer`、`RemoteTransportVTable` + `route_remote_signal`；本地地址被显式拒绝 |
| H.4 形式化验证 | ✅ | 新 crate `gos-verify`：`pick_u32_sweep` + `assume_pred!` 双模式（Kani feature on / off），首批 invariants：charge_heap 单调、credit_heap 不下溢 |
...
- H.5.1 — snapshot 写盘 + boot 时优先于 journal replay 加载
