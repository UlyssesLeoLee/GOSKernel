...
| 版本 | 日期 | 变更内容 | 作成者 |
|---|---|---|---|
| v1.0 | 2026-04-28 | PluginGroup 架构重构后系统测试报告初版 | GOS 核心团队 |
| v1.1 | 2026-07-01 | 纳入日系工程阶段目录（05_测试验证）；补充文档管理信息 | GOS 核心团队 |

> **口径说明**：本报告基线为 2026-04-28（21 模块、17 milestone）。V2.0~V2.14 硬化系列（2026-06-30 起）新增的 `nodes`/`edges`/`graph diff`/`proc` 等能力均附带独立 host-test harness（回归统计见各硬化日志与 [GOS测试报告书.xlsx](./GOS测试报告书.xlsx)），但尚未产出新的端到端 QEMU 系统测试报告。建议下一次系统级验收（QEMU boot + 键盘 IRQ 用户验证）时更新本文档或新增 v2 报告，避免"最新架构状态"与"最新系统测试基线"日期脱节。
