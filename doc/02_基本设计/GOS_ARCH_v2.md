...
| v2.8 | 2026-07-21 | §5.1 补充 V3.66~V3.104（39 项硬化日志，其中 V3.66/V3.102 文件缺失）新增的 Neighborhood S-variant 拓扑指数命令族 `graph topo55`~`graph topo93`（39 组、117 个指数）概述，指向 [GRAPH_CLI_COMMANDS_zh.md §十五](../03_详细设计/GRAPH_CLI_COMMANDS_zh.md) 作为权威口径；累计 host tests 由 1623 更新为约 2021（截至 V3.104）；同步将区间内 16 篇纯英文/双语硬化日志（V3.77/78/84/85/86/87/88/89/90/93/94/95/97/98/99/100）就地中文化；如实标注 V3.66/V3.102 硬化日志文件缺失、V3.99→V3.100 累计测试数口径不一致两处已知缺口，未擅自修正 | GOS 核心团队 |
| v2.9 | 2026-07-29 | §5.1 补充 V3.105~V3.125（21 项硬化迭代，其中 V3.110/V3.111/V3.113 三版本文件缺失）新增的 Neighborhood S-variant 拓扑指数命令族 `graph topo94`~`graph topo114`（21 组、63 个指数）概述，指向 [GRAPH_CLI_COMMANDS_zh.md §十五](../03_详细设计/GRAPH_CLI_COMMANDS_zh.md)（已扩展至 v1.9）作为权威口径；累计 host tests 由约 2021 更新为约 2221（截至 V3.125）；用 `git log --all --oneline` 核实 V3.110/V3.111/V3.113 与此前已知的 V3.66/V3.102 共 5 处硬化日志文件缺失，均为对应 `feat` 提交已合入但从未生成 `docs` 归档提交，属文档编写环节遗漏，非代码或测试缺失，解除上一轮「待核实」标记；如实标注 V3.117 硬化日志自述累计测试数与 V3.116 重复（应递增未递增）的口径小瑕疵，未擅自修正 | GOS 核心团队 |
| v2.10 | 2026-07-30 | 修正本表 v2.8/v2.9 两行此前的顺序颠倒（应按日期升序排列）；§5.1 补充 V3.126~V3.128（3 项硬化迭代）新增的 `graph topo115`~`graph topo117`（3 组、9 个指数）概述，指向 [GRAPH_CLI_COMMANDS_zh.md §十五](../03_详细设计/GRAPH_CLI_COMMANDS_zh.md)（已扩展至 v1.11）作为权威口径；累计 host tests 由约 2221 更新为约 2251（截至 V3.128）；V3.127 硬化日志原误存于 `docs/hardening/`（非本文档体系 `doc/`），本轮归位；V3.128 无任何归档提交，本轮首次尝试依据源码实现与测试套件重建硬化日志（而非仅标注缺失），详见 GRAPH_CLI_COMMANDS_zh.md §十五说明 | GOS 核心团队 |
...
V3.66 起，Neighborhood S-variant 拓扑指数命令族继续沿顶点/边幂次序列与 Sombor α 序列扩展（V3.66~V3.104，39 次硬化迭代，其中 V3.66、V3.102 两版本文件缺失，详见 [GRAPH_CLI_COMMANDS_zh.md §十五](../03_详细设计/GRAPH_CLI_COMMANDS_zh.md) 的缺口说明），新增 `graph topo55`~`graph topo93` 共 39 组、117 个指数：顶点/边幂次由 30 次幂推进至 67 次幂，依次跨越 triacontic（30余次幂）尾段、tetracontic（40余次幂）段、pentacontic（50余次幂）段、hexacontic（60余次幂）段前 8 个；Sombor 变体 α 由 46 推进至 122，命名法历经第2轮单字母复用（NASO~NZSO）、第3轮双字母 "AA"~"AZ"、第4轮双字母 "BA"~"BJ" 三次进位。本轮同时将 V3.66~V3.104 区间内 16 篇纯英文/双语硬化日志（V3.77/78/84/85/86/87/88/89/90/93/94/95/97/98/99/100）就地中文化，仅译语言，不改动版本号/文件路径/函数名/测试名/测试计数。至此宿主测试总数累计约 **2021 个**（截至 V3.104，具体数值另见硬化日志逐版本累计数，其中 V3.99→V3.100 的自述累计数存在 10 项误差，已在硬化日志译文与 GRAPH_CLI_COMMANDS_zh.md §十五中如实标注、未擅自改动）。完整命令语法、别名、逐组对应关系与公式推导以 [GRAPH_CLI_COMMANDS_zh.md §十五](../03_详细设计/GRAPH_CLI_COMMANDS_zh.md) 为唯一权威口径，本文档不重复维护逐条命令细节。

V3.105 起，Neighborhood S-variant 拓扑指数命令族继续沿顶点/边幂次序列与 Sombor α 序列扩展（V3.105~V3.125，21 次硬化迭代，其中 V3.110、V3.111、V3.113 三版本文件缺失，均已用 `git log --all --oneline` 核实为对应 `feat` 提交已合入但从未生成 `docs` 归档提交的文档编写遗漏，非代码或测试缺失），新增 `graph topo94`~`graph topo114` 共 21 组、63 个指数：顶点/边幂次由 68 次幂推进至 88 次幂，先后收官 hexacontic（60余次幂）、heptacontic（70余次幂）两段，并开启 octacontic（80余次幂）段；Sombor 变体 α 由 124 推进至 164，命名标识经历单字母 K~Z 收尾后，自 topo110（V3.121）起切换为硬化日志自述的 "AA"、单字母 "B"、重复双字母 "CC"/"DD"/"EE" 等标识（本文档仅转录硬化日志自述结果，不代为归纳统一规则）。本轮同时发现 V3.117（topo106）硬化日志自述累计测试数与 V3.116（topo105）完全相同（应递增 10 而未递增），经与 V3.118 起始基数交叉核对，确认仅为落款时的复制遗漏，未影响后续版本真实累计数，如实标注不擅自修正。至此宿主测试总数累计约 **2221 个**（截至 V3.125）。完整命令语法、别名、逐组对应关系与公式推导以 [GRAPH_CLI_COMMANDS_zh.md §十五](../03_详细设计/GRAPH_CLI_COMMANDS_zh.md) 为唯一权威口径，本文档不重复维护逐条命令细节。

V3.126 起，Neighborhood S-variant 拓扑指数命令族收官 octacontic（80余次幂）段并开启 ennacontic（90余次幂）段（V3.126~V3.128，3 次硬化迭代），新增 `graph topo115`~`graph topo117` 共 3 组、9 个指数：顶点/边幂次由 89 次幂推进至 91 次幂，Sombor 变体 α 由 166 推进至 170（NB 系列第 32~34 组，标识 "FF"/"GG"/"HH"）。其中 V3.127 硬化日志原稿误存于仓库 `docs/hardening/` 目录（与本文档体系的 `doc/` 根目录同名但非同一路径，命名格式亦不合规范），本轮已转录归位；V3.128 在仓库中无任何形式的归档记录（既无 `doc/`/`docs/` 下的 markdown 文件，也无 `docs(v3.128)` 提交），本轮依据其源码实现（`gos-runtime::graph_topo_indices117_inner`）与测试套件（`gos-graph-topo117-harness`）重建了硬化日志，为首次尝试以「源码 + 测试」代替「仅标注缺失」处理此类缺口，重建性质已在文中如实标注。至此宿主测试总数累计约 **2251 个**（截至 V3.128）。完整命令语法、别名、逐组对应关系与公式推导以 [GRAPH_CLI_COMMANDS_zh.md §十五](../03_详细设计/GRAPH_CLI_COMMANDS_zh.md) 为唯一权威口径，本文档不重复维护逐条命令细节。
