...
### B.4.1 — Domain PML4 构造

- `k-vmm::clone_kernel_pml4()` + 单元测试（host harness 用 mock PML4）
- `Supervisor::map_module` 调用并写 `domain.root_table_phys`
- 验证：所有 builtin 模块的 `domain.root_table_phys` 非零
...
### B.4.3 — IST stack & fault routing

- `k-idt` 注册 4 张 IST 栈
- `k-pf-handler` (新模块或 k-idt 子模块): #PF/#GP/#DF/#SS handler 走 IST
- handler 内部：读 `gos_runtime::current_dispatch_instance()` → 调用 `gos_supervisor::fault_instance()` → 不返回原栈
- 验证：构造一个故意触发 #PF 的测试 plugin，观察其被 degrade、`where` 显示 DEGRADED
