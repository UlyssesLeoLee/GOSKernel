...
> 状态：**§五第 1-3 项（选项无关的地基工作）已落地（2026-08-05）；fork/exec/信号本体仍待你在 §二/§三选向** · 提案日期：2026-08-05 · 配套：[ADR-014](./ADR-014-process-as-subgraph-compat-strategy.md)（§二进程=子图映射已落地；§六记录选向 B，本 ADR 是 B 自己要求的"各自独立 ADR"里第一个动笔的）、[ADR-018](./ADR-018-bootloader-uefi-migration.md)（真实 ELF header 解析证实内核当前加载在低半区 `0x200000`，是本 ADR §一发现的关键阻塞项的直接证据）
>
...
2. **`CreateNode` 参数化**——✅ **已落地（2026-08-05）**。§1.4 的独立小缺口。`CypherMutation::CreateNode` 从无字段变体改为 `{ node_type, entry_policy, executor_id }`，一路穿透 `MutationDispatcher::create_node` → `gos_runtime::create_provisional_node` → `create_provisional_node_locked`；所有现有调用方（k-shell 的 `gpm_install`、k-cypher 的 `CREATE (n)`、gos-ai-bridge 的 `create_node` 线协议）现在显式传 `Vector`/`Manual`/`ZERO`，行为不变。`gos_supervisor::apply_cypher_mutation`/`gos_runtime::apply_edge_mutation` 仍然整体拒绝 `CreateNode`（谁能请求非 Vector 节点类型是另一个更晚的策略问题，本项不解决），所以这一项本身不授予任何新能力。顺带把 `gos-shadow-kernel-harness` 里一个明确写着"这是已知未闭合缺口"的测试（`capability_granting_mutation_is_the_promote_trigger_adr014`，注释原话"`proc` stands in for it"）改成真的铸造 §2.1 要求的 `Compute`/`OnDemand`/真实 `ExecutorId` 形状，缺口闭合。验证：`gos-kernel` QEMU+OVMF 干净启动（CreateNode 不在启动路径上，未受影响）；`gos-runtime-harness` 47/47、`gos-shadow-kernel-harness` 19/19 host test 全过；`verify-graph-architecture.ps1` 干净。详见 commit `fc4c19a`。
3. **验证 ring3 syscall trampoline 真的工作**——✅ **已落地（2026-08-05）**。新增 `crates/gos-kernel/src/ring3_probe.rs`：往一个 `USER_ACCESSIBLE` 页里写几个字节的内联 shellcode（`mov eax, 哨兵号; syscall; jmp $`），手写 `iretq` 帧把 CPU 真的降到 CPL3（RFLAGS.IF 强制清零——这一步跑在 `interrupts::enable()` 之前，且 `k_gdt` 只填了 4 个 IST 栈槽，`TSS.RSP0` 从未设置，CPL3 下若真有可屏蔽中断打进来会没有安全栈可用），执行真实的 `syscall`，确认 `ring3::rust_syscall_handler` 真的接住了——在此之前，从未有任何代码在任何一次启动中真正从 ring3 发出过 `syscall`。
   第一次跑直接把内核挂死（静默自旋，QEMU 加 `-d int,cpu_reset` 手动起一次也没有任何异常向量、探针启动后 `0xffffffff80xxxxxx` 地址范围再无任何活动）。定位到两个 bug，一个在探针自己，一个是 `ring3.rs` 里预先存在、从未被跑到过的真实 bug：
   - 探针自己的 shellcode 忘了在 `syscall` 前把哨兵号真的写进 RAX。
   - `ring3::rust_syscall_handler` 的形参**名字**就是 `rax`/`rdi`/`rsi`/`rdx`/`rcx`/`r8`，但 `syscall_entry` 的汇编只把 syscall ABI 第 4 个参数（R10）挪进了 RCX 就直接 `call` 了——按真实 C 调用约定，handler 读到的每个"寄存器"其实都是 syscall ABI 里下一个寄存器的值（"rax" 实际收到的是 RDI 的值，"rdi" 收到的是 RSI 的值……），声明的第 7 个参数背后既没有寄存器也没有栈槽。修法：`syscall_entry` 显式把 `(RAX,RDI,RSI,RDX,R10,R8)` 搬到 C ABI 的 `(RDI,RSI,RDX,RCX,R8,R9)`，`rust_syscall_handler` 的形参改名为其真实语义（`syscall_no, arg0..arg3`）。
   验证：QEMU+OVMF 实测 `ring3 syscall trampoline probe: PASS`，启动其余部分（CR3 trampoline 校验、steady-state、desktop init）不受影响；`verify-graph-architecture.ps1` 干净（`KERNEL_RESUME_RSP` 用 `AtomicU64` 而非 `static mut`，满足仓库的 no-static-mut 规则）。详见 commit `6e97f8a`。
