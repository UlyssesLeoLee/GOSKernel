...
// ── Phase I.1.2 — fuzz-style decoder hardening ─────────────────────
//
// The bridge decoder runs on host code that processes bytes the
// kernel emits across a (future) IPC boundary; a kernel-side bug or
// a malicious actor can hand it ANY byte sequence.  The contract:
// `read_bridge_frame` MUST always return a typed `Result` — never
// panic, never UB, never read past the slice.  This test feeds
// 25 000 deterministic-PRNG-generated byte buffers of varying length
// and asserts that contract; only `DecodeFailed` is the legal
// failure mode for an unstructured stream.
//
// Deterministic over a fixed seed so a CI failure reproduces locally
// from the seed alone.  CI nightly will run a real `cargo fuzz`
// target on top of this (PHASE_I_GRAPHICS.md I.4.7); the property
// loop here is the always-on baseline that fires on every PR.
...
fn decoder_never_panics_on_random_input() {
    ...
    const SEED: u32 = 0x9E37_79B9; // golden-ratio nothing-up-my-sleeve

    let mut rng = SEED;
    let mut buf = vec![0u8; MAX_LEN];
    let mut ok_count: u32 = 0;
    let mut decode_failed_count: u32 = 0;

    for iter in 0..ITERATIONS {
        let len = (xorshift32(&mut rng) as usize) % (MAX_LEN + 1);
        for slot in buf[..len].iter_mut() {
            *slot = (xorshift32(&mut rng) & 0xFF) as u8;
        }

        // Periodically inject a known-valid prefix so the decoder
        // sometimes succeeds, exercising the "consumed length" path
        // in addition to the rejection paths.
        if iter % 97 == 0 && len >= BRIDGE_FRAME_HEADER_BYTES + 4 {
            let mut staging = [0u8; 32];
            let n = write_bridge_frame(
                &mut staging,
                &RenderCommand::BeginFrame {
                    surface: SurfaceId(iter as u32),
                },
            )
            .expect("encode valid prefix");
            buf[..n].copy_from_slice(&staging[..n]);
        }

        match read_bridge_frame(&buf[..len]) {
            Ok((_, consumed)) => {
                assert!(consumed <= len, "decoder claimed to consume past slice end");
                ok_count += 1;
            }
            Err(GfxError::DecodeFailed) => {
                decode_failed_count += 1;
            }
            Err(other) => {
                panic!(
                    "decoder returned non-DecodeFailed error on random input: {:?} (iter {}, len {})",
                    other, iter, len
                );
            }
        }
    ...
    // Sanity floor: random bytes should mostly fail to decode (the
    // 16-byte header alone has a ~2^-32 chance of a valid CRC over
    // random magic/version), but the periodic valid-prefix injection
    // every 97 iters should give us at least a handful of Ok results.
    assert!(
        ok_count >= 50,
        "expected periodic valid-prefix injection to land >=50 Ok results; got {}",
        ok_count
    );
    assert!(
        decode_failed_count + ok_count == ITERATIONS as u32,
        "every iteration must classify cleanly"
    );
}
