...
// `RuntimeDispatcher` (the Cypher-mutation-dispatch path) derives edge ids
// under the "cypher."-prefixed key convention (`MUTATION_USE_EDGE_KEY` /
// `MUTATION_MOUNT_EDGE_KEY` in gos-runtime), distinct from the bare "use" /
// "mount" keys `rebind_exclusive_use` and boot-time edges use — see
// `doc/ADR-004`'s §四.1 note on edge-key convention fragmentation.
fn dispatcher_use_edge_present(from: NodeId, to: NodeId) -> bool {
    let id = derive_edge_id(from, to, "cypher.use");
    gos_runtime::edge_id_for_vector(derive_edge_vector(id)).is_some()
}

fn dispatcher_mount_edge_present(from: NodeId, to: NodeId) -> bool {
    let id = derive_edge_id(from, to, "cypher.mount");
    gos_runtime::edge_id_for_vector(derive_edge_vector(id)).is_some()
}
...
fn rebind_use_is_atomic_single_epoch_and_exclusive() {
    ...
    assert!(dispatcher_use_edge_present(theme_current, wabi));
    assert!(!dispatcher_use_edge_present(theme_current, shoji));
    ...
    assert!(dispatcher_mount_edge_present(a, b), "Mount edge should be in the live table");
    let mount_id = derive_edge_id(a, b, "cypher.mount");
    d.remove_edge(mount_id).expect("dispatcher remove_edge");
    assert!(!dispatcher_mount_edge_present(a, b), "Mount edge should be removed");
