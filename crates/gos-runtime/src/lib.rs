...
impl GraphRuntime {
    ...
    pub fn graph_topo_indices67_inner(&self) -> (u64, u64, u64, usize, usize) {
        ...
        // 5. Vertex scan: NHENTETRAACTC (S-Hentetracontic vertex sum = Σ_v S(v)^41).
        //
        //    NHENTETRAACTC(G) = Σ_v S(v)^41  (exact u128→u64; S-Hentetracontic vertex sum)
        //
        //    Extends the S-power-vertex series:
        //      NM₁=Σ S² (topo18) → ... → NTETRAACTC=Σ S⁴⁰ (topo66) → NHENTETRAACTC=Σ S⁴¹ (topo67)
        ...
        let mut nhentetraactc_acc: u128 = 0;
        for ci in 0..nc {
            let s   = sv[ci] as u128;
            let s2  = s * s;
            let s4  = s2 * s2;
            ...
            let s16 = s8.saturating_mul(s8);
            let s32 = s16.saturating_mul(s16);
            let s41 = s32.saturating_mul(s8).saturating_mul(s);
            nhentetraactc_acc = nhentetraactc_acc.saturating_add(s41);
        }
        let nhentetraactc = nhentetraactc_acc.min(u64::MAX as u128) as u64;

        // 6. Edge scan (a < b): NHHENTETRAACTC (S-Tetracontic edge-sum) and NAJSO (S-Tetracontyl Sombor).
        //
        //    NHHENTETRAACTC(G) = Σ_{uv∈E} (S_u+S_v)^40  (exact u128→u64; S-Tetracontic edge-sum)
        //    Extends the S-power-edge series:
        //      NHM1=Σ(S+S)² (topo23) → ... → NHTETRAACTC=Σ(S+S)³⁹ (topo66)
        //      → NHHENTETRAACTC=Σ(S+S)⁴⁰ (topo67)
        ...
        //
        //    NAJSO(G) = Σ_{uv∈E} (S_u²+S_v²)^35  (exact u128→u64; S-Tetracontyl Sombor α=70)
        //    S-variant generalised Sombor SO^α with α=70: exact integer (no isqrt).
        //    NSO(α=1), NCSO(α=3), NFSO(α=4), NHSO(α=6), NOSO(α=8), NTSO(α=10),
        //    NDSO(α=12), NESO(α=14), NGSO(α=16), NIOSO(α=18), NJSO(α=20), NKSO(α=22),
        //    NLSO(α=24), NMSO(α=26), NNSO(α=28), NPSO(α=30), NQSO(α=32), NRSO(α=34),
        //    NSSO(α=36), NUSO(α=38), NVSO(α=40), NXSO(α=42), NYSO(α=44), NZSO(α=46),
        //    NASO(α=48), NBSO(α=50), NAASO(α=52), NABSO(α=54), NACSO(α=56), NADSO(α=58),
        //    NAESO(α=60), NAFSO(α=62), NAGSO(α=64), NAHSO(α=66), NAISO(α=68), NAJSO(α=70). (3rd-pass AJ.)
        //    NAJSO = |E|·(2S²)^35 = 34359738368|E|·S^70 for S-regular.
        //    Overflow per edge: (2×16129²)^35 → saturating u128 accumulator.
        //    Implementation: s2s^35 = s2s32 × s2s2 × s2s  (s2s32=s2s16^2; 35=32+2+1).

        let mut nhhentetraactc_acc: u128 = 0;
        let mut najso_acc:           u128 = 0;
        for a in 0..nc {
            let sa  = sv[a] as u128;
            let mut bits = adj[a];
            while bits != 0 {
                ...
                if b > a {
                    let sb     = sv[b] as u128;
                    // NHHENTETRAACTC: (S_a + S_b)^40 = ss32 × ss8  (40=32+8, sum of two powers of 2)
                    let ss     = sa + sb;
                    let ss2    = ss * ss;
                    let ss4    = ss2 * ss2;
                    let ss8    = ss4.saturating_mul(ss4);
                    let ss16   = ss8.saturating_mul(ss8);
                    let ss32   = ss16.saturating_mul(ss16);
                    let ss40   = ss32.saturating_mul(ss8);
                    nhhentetraactc_acc = nhhentetraactc_acc.saturating_add(ss40);
                    // NAJSO: (S_a² + S_b²)^35 = s2s32 × s2s2 × s2s  (35=32+2+1)
                    let s2s    = sa * sa + sb * sb;
                    let s2s2   = s2s * s2s;
                    let s2s4   = s2s2.saturating_mul(s2s2);
                    let s2s8   = s2s4.saturating_mul(s2s4);
                    let s2s16  = s2s8.saturating_mul(s2s8);
                    let s2s32  = s2s16.saturating_mul(s2s16);
                    let s2s35  = s2s32.saturating_mul(s2s2).saturating_mul(s2s);
                    najso_acc = najso_acc.saturating_add(s2s35);
                }
            }
        }
        let nhhentetraactc = nhhentetraactc_acc.min(u64::MAX as u128) as u64;
        let najso          = najso_acc.min(u64::MAX as u128) as u64;
    ...
    pub fn graph_topo_indices68_inner(&self) -> (u64, u64, u64, usize, usize) {
        ...
        // 5. Vertex scan: NDOTETRAACTC (S-Dotetracontic vertex sum = Σ_v S(v)^42).
        //
        //    NDOTETRAACTC(G) = Σ_v S(v)^42  (exact u128→u64; S-Dotetracontic vertex sum)
        //
        //    Extends the S-power-vertex series:
        //      NM₁=Σ S² (topo18) → ... → NHENTETRAACTC=Σ S⁴¹ (topo67) → NDOTETRAACTC=Σ S⁴² (topo68)
        ...
        let mut ndotetraactc_acc: u128 = 0;
        for ci in 0..nc {
            let s   = sv[ci] as u128;
            let s2  = s * s;
            let s4  = s2 * s2;
            ...
            let s16 = s8.saturating_mul(s8);
            let s32 = s16.saturating_mul(s16);
            let s42 = s32.saturating_mul(s8).saturating_mul(s2);
            ndotetraactc_acc = ndotetraactc_acc.saturating_add(s42);
        }
        let ndotetraactc = ndotetraactc_acc.min(u64::MAX as u128) as u64;

        // 6. Edge scan (a < b): NHDOTETRAACTC (S-Hentetracontic edge-sum) and NAKSO (S-Dotetracontyl Sombor).
        //
        //    NHDOTETRAACTC(G) = Σ_{uv∈E} (S_u+S_v)^41  (exact u128→u64; S-Hentetracontic edge-sum)
        //    Extends the S-power-edge series:
        //      NHM1=Σ(S+S)² (topo23) → ... → NHHENTETRAACTC=Σ(S+S)⁴⁰ (topo67)
        ...
        //
        //    NAKSO(G) = Σ_{uv∈E} (S_u²+S_v²)^36  (exact u128→u64; S-Dotetracontyl Sombor α=72)
        //    S-variant generalised Sombor SO^α with α=72: exact integer (no isqrt).
        //    NSO(α=1), NCSO(α=3), NFSO(α=4), NHSO(α=6), NOSO(α=8), NTSO(α=10),
        //    NDSO(α=12), NESO(α=14), NGSO(α=16), NIOSO(α=18), NJSO(α=20), NKSO(α=22),
        //    NLSO(α=24), NMSO(α=26), NNSO(α=28), NPSO(α=30), NQSO(α=32), NRSO(α=34),
        //    NSSO(α=36), NUSO(α=38), NVSO(α=40), NXSO(α=42), NYSO(α=44), NZSO(α=46),
        //    NASO(α=48), NBSO(α=50), NAASO(α=52), NABSO(α=54), NACSO(α=56), NADSO(α=58),
        //    NAESO(α=60), NAFSO(α=62), NAGSO(α=64), NAHSO(α=66), NAISO(α=68), NAJSO(α=70),
        //    NAKSO(α=72). (3rd-pass AK.)
        //    NAKSO = |E|·(2S²)^36 = 68719476736|E|·S^72 for S-regular.
        //    Overflow per edge: (2×16129²)^36 → saturating u128 accumulator.
        //    Implementation: s2s^36 = s2s32 × s2s4  (s2s32=s2s16^2; s2s4=s2s2^2; 36=32+4).

        let mut nhdotetraactc_acc: u128 = 0;
        let mut nakso_acc:          u128 = 0;
        for a in 0..nc {
            let sa  = sv[a] as u128;
            let mut bits = adj[a];
            while bits != 0 {
                ...
                if b > a {
                    let sb     = sv[b] as u128;
                    // NHDOTETRAACTC: (S_a + S_b)^41 = ss32 × ss8 × ss  (41=32+8+1)
                    let ss     = sa + sb;
                    let ss2    = ss * ss;
                    let ss4    = ss2 * ss2;
                    let ss8    = ss4.saturating_mul(ss4);
                    let ss16   = ss8.saturating_mul(ss8);
                    let ss32   = ss16.saturating_mul(ss16);
                    let ss41   = ss32.saturating_mul(ss8).saturating_mul(ss);
                    nhdotetraactc_acc = nhdotetraactc_acc.saturating_add(ss41);
                    // NAKSO: (S_a² + S_b²)^36 = s2s32 × s2s4  (36=32+4, sum of two powers of 2)
                    let s2s    = sa * sa + sb * sb;
                    let s2s2   = s2s * s2s;
                    let s2s4   = s2s2.saturating_mul(s2s2);
                    let s2s8   = s2s4.saturating_mul(s2s4);
                    let s2s16  = s2s8.saturating_mul(s2s8);
                    let s2s32  = s2s16.saturating_mul(s2s16);
                    let s2s36  = s2s32.saturating_mul(s2s4);
                    nakso_acc = nakso_acc.saturating_add(s2s36);
                }
            }
        }
        let nhdotetraactc = nhdotetraactc_acc.min(u64::MAX as u128) as u64;
        let nakso         = nakso_acc.min(u64::MAX as u128) as u64;
    ...
    pub fn graph_topo_indices69_inner(&self) -> (u64, u64, u64, usize, usize) {
        ...
        // 5. Vertex scan: NTRITETRAACTC (S-Tritetracontic vertex sum = Σ_v S(v)^43).
        //
        //    NTRITETRAACTC(G) = Σ_v S(v)^43  (exact u128→u64; S-Tritetracontic vertex sum)
        //
        //    Extends the S-power-vertex series:
        //      ... → NDOTETRAACTC=Σ S⁴² (topo68) → NTRITETRAACTC=Σ S⁴³ (topo69)
        //    NTRITETRAACTC = n·S^43 for S-regular.
        //    Overflow: S^43 > u64::MAX for S≥2 → saturating u128 accumulator, clamp to u64::MAX.
        //    Implementation: s^43 = s32 × s8 × s2 × s  (43=32+8+2+1).

        let mut ntritetraactc_acc: u128 = 0;
        for ci in 0..nc {
            let s   = sv[ci] as u128;
            let s2  = s * s;
            let s4  = s2 * s2;
            ...
            let s16 = s8.saturating_mul(s8);
            let s32 = s16.saturating_mul(s16);
            let s43 = s32.saturating_mul(s8).saturating_mul(s2).saturating_mul(s);
            ntritetraactc_acc = ntritetraactc_acc.saturating_add(s43);
        }
        let ntritetraactc = ntritetraactc_acc.min(u64::MAX as u128) as u64;

        // 6. Edge scan (a < b): NHTRITETRAACTC (S-Dotetracontic edge-sum) and NALSO (S-Tritetracontyl Sombor).
        //
        //    NHTRITETRAACTC(G) = Σ_{uv∈E} (S_u+S_v)^42  (exact u128→u64; S-Dotetracontic edge-sum)
        //    Extends the S-power-edge series:
        ...
        //    Implementation: ss^42 = ss32 × ss8 × ss2  (42=32+8+2).
        //
        //    NALSO(G) = Σ_{uv∈E} (S_u²+S_v²)^37  (exact u128→u64; S-Tritetracontyl Sombor α=74)
        //    S-variant generalised Sombor SO^α with α=74: exact integer (no isqrt).
        //    NAASO(α=52)→...→NAKSO(α=72,topo68)→NALSO(α=74,topo69). (3rd-pass AL.)
        //    NALSO = |E|·(2S²)^37 = 137438953472|E|·S^74 for S-regular.
        //    Overflow per edge: (2×16129²)^37 → saturating u128 accumulator.
        //    Implementation: s2s^37 = s2s32 × s2s4 × s2s  (37=32+4+1).

        let mut nhtritetraactc_acc: u128 = 0;
        let mut nalso_acc:           u128 = 0;
        for a in 0..nc {
            let sa  = sv[a] as u128;
            let mut bits = adj[a];
            while bits != 0 {
                ...
                if b > a {
                    let sb     = sv[b] as u128;
                    // NHTRITETRAACTC: (S_a + S_b)^42 = ss32 × ss8 × ss2  (42=32+8+2)
                    let ss     = sa + sb;
                    let ss2    = ss * ss;
                    let ss4    = ss2 * ss2;
                    let ss8    = ss4.saturating_mul(ss4);
                    let ss16   = ss8.saturating_mul(ss8);
                    let ss32   = ss16.saturating_mul(ss16);
                    let ss42   = ss32.saturating_mul(ss8).saturating_mul(ss2);
                    nhtritetraactc_acc = nhtritetraactc_acc.saturating_add(ss42);
                    // NALSO: (S_a² + S_b²)^37 = s2s32 × s2s4 × s2s  (37=32+4+1)
                    let s2s    = sa * sa + sb * sb;
                    let s2s2   = s2s * s2s;
                    let s2s4   = s2s2.saturating_mul(s2s2);
                    let s2s8   = s2s4.saturating_mul(s2s4);
                    let s2s16  = s2s8.saturating_mul(s2s8);
                    let s2s32  = s2s16.saturating_mul(s2s16);
                    let s2s37  = s2s32.saturating_mul(s2s4).saturating_mul(s2s);
                    nalso_acc = nalso_acc.saturating_add(s2s37);
                }
            }
        }
        let nhtritetraactc = nhtritetraactc_acc.min(u64::MAX as u128) as u64;
        let nalso          = nalso_acc.min(u64::MAX as u128) as u64;
    ...
    pub fn graph_topo_indices70_inner(&self) -> (u64, u64, u64, usize, usize) {
        ...
        // 5. Vertex scan: NTETRATETRAACTC (S-Tetratetracontic vertex sum = Σ_v S(v)^44).
        //
        //    NTETRATETRAACTC(G) = Σ_v S(v)^44  (exact u128→u64; S-Tetratetracontic vertex sum)
        //
        //    Extends the S-power-vertex series:
        //      ... → NTRITETRAACTC=Σ S⁴³ (topo69) → NTETRATETRAACTC=Σ S⁴⁴ (topo70)
        //    NTETRATETRAACTC = n·S^44 for S-regular.
        //    Overflow: S^44 > u64::MAX for S≥2 → saturating u128 accumulator, clamp to u64::MAX.
        //    Implementation: s^44 = s32 × s8 × s4  (44=32+8+4; 3 mults).

        let mut ntetratetraactc_acc: u128 = 0;
        for ci in 0..nc {
            let s   = sv[ci] as u128;
            let s2  = s * s;
            let s4  = s2 * s2;
            ...
            let s16 = s8.saturating_mul(s8);
            let s32 = s16.saturating_mul(s16);
            let s44 = s32.saturating_mul(s8).saturating_mul(s4);
            ntetratetraactc_acc = ntetratetraactc_acc.saturating_add(s44);
        }
        let ntetratetraactc = ntetratetraactc_acc.min(u64::MAX as u128) as u64;

        // 6. Edge scan (a < b): NHTETRATETRAACTC (S-Tritetracontic edge-sum) and NAMSO (S-Tetratetracontyl Sombor).
        //
        //    NHTETRATETRAACTC(G) = Σ_{uv∈E} (S_u+S_v)^43  (exact u128→u64; S-Tritetracontic edge-sum)
        //    Extends the S-power-edge series:
        //      → NHTRITETRAACTC=Σ(S+S)⁴² (topo69) → NHTETRATETRAACTC=Σ(S+S)⁴³ (topo70)
        //    NHTETRATETRAACTC = |E|·(2S)^43 = 8796093022208|E|·S^43 for S-regular.
        //    Overflow per edge: (2×16129)^43 → saturating u128 accumulator.
        //    Implementation: ss^43 = ss32 × ss8 × ss2 × ss  (43=32+8+2+1).
        //
        //    NAMSO(G) = Σ_{uv∈E} (S_u²+S_v²)^38  (exact u128→u64; S-Tetratetracontyl Sombor α=76)
        //    S-variant generalised Sombor SO^α with α=76: exact integer (no isqrt).
        //    NAASO(α=52)→...→NALSO(α=74,topo69)→NAMSO(α=76,topo70). (3rd-pass AM.)
        //    NAMSO = |E|·(2S²)^38 = 274877906944|E|·S^76 for S-regular.
        //    Overflow per edge: (2×16129²)^38 → saturating u128 accumulator.
        //    Implementation: s2s^38 = s2s32 × s2s4 × s2s2  (38=32+4+2).

        let mut nhtetratetraactc_acc: u128 = 0;
        let mut namso_acc:             u128 = 0;
        for a in 0..nc {
            let sa  = sv[a] as u128;
            let mut bits = adj[a];
            while bits != 0 {
                ...
                if b > a {
                    let sb     = sv[b] as u128;
                    // NHTETRATETRAACTC: (S_a + S_b)^43 = ss32 × ss8 × ss2 × ss  (43=32+8+2+1)
                    let ss     = sa + sb;
                    let ss2    = ss * ss;
                    let ss4    = ss2 * ss2;
                    let ss8    = ss4.saturating_mul(ss4);
                    let ss16   = ss8.saturating_mul(ss8);
                    let ss32   = ss16.saturating_mul(ss16);
                    let ss43   = ss32.saturating_mul(ss8).saturating_mul(ss2).saturating_mul(ss);
                    nhtetratetraactc_acc = nhtetratetraactc_acc.saturating_add(ss43);
                    // NAMSO: (S_a² + S_b²)^38 = s2s32 × s2s4 × s2s2  (38=32+4+2)
                    let s2s    = sa * sa + sb * sb;
                    let s2s2   = s2s * s2s;
                    let s2s4   = s2s2.saturating_mul(s2s2);
                    let s2s8   = s2s4.saturating_mul(s2s4);
                    let s2s16  = s2s8.saturating_mul(s2s8);
                    let s2s32  = s2s16.saturating_mul(s2s16);
                    let s2s38  = s2s32.saturating_mul(s2s4).saturating_mul(s2s2);
                    namso_acc = namso_acc.saturating_add(s2s38);
                }
            }
        }
        let nhtetratetraactc = nhtetratetraactc_acc.min(u64::MAX as u128) as u64;
        let namso            = namso_acc.min(u64::MAX as u128) as u64;
    ...
    pub fn graph_topo_indices71_inner(&self) -> (u64, u64, u64, usize, usize) {
        ...
        // 5. Vertex scan: NPENTETRAACTC (S-Pentatetracontic vertex sum = Σ_v S(v)^45).
        //
        //    NPENTETRAACTC(G) = Σ_v S(v)^45  (exact u128→u64; S-Pentatetracontic vertex sum)
        //
        //    Extends: ... → NTETRATETRAACTC=Σ S⁴⁴ (topo70) → NPENTETRAACTC=Σ S⁴⁵ (topo71)
        //    NPENTETRAACTC = n·S^45 for S-regular.
        //    Overflow: S^45 > u64::MAX for S≥2 → saturating u128 accumulator, clamp to u64::MAX.
        //    Implementation: s^45 = s32 × s8 × s4 × s  (45=32+8+4+1; 4 mults).

        let mut npentetraactc_acc: u128 = 0;
        for ci in 0..nc {
            let s   = sv[ci] as u128;
            let s2  = s * s;
            let s4  = s2 * s2;
            ...
            let s16 = s8.saturating_mul(s8);
            let s32 = s16.saturating_mul(s16);
            let s45 = s32.saturating_mul(s8).saturating_mul(s4).saturating_mul(s);
            npentetraactc_acc = npentetraactc_acc.saturating_add(s45);
        }
        let npentetraactc = npentetraactc_acc.min(u64::MAX as u128) as u64;

        // 6. Edge scan (a < b): NHPENTETRAACTC (S-Tetratetracontic edge-sum) and NANSO (S-Pentatetracontyl Sombor).
        //
        //    NHPENTETRAACTC(G) = Σ_{uv∈E} (S_u+S_v)^44  (exact u128→u64; S-Tetratetracontic edge-sum)
        //    Extends: → NHTETRATETRAACTC=Σ(S+S)⁴³ (topo70) → NHPENTETRAACTC=Σ(S+S)⁴⁴ (topo71)
        //    NHPENTETRAACTC = |E|·(2S)^44 = 17592186044416|E|·S^44 for S-regular.
        //    Overflow per edge: (2×16129)^44 → saturating u128 accumulator.
        //    Implementation: ss^44 = ss32 × ss8 × ss4  (44=32+8+4; 3 mults — efficient!).
        //
        //    NANSO(G) = Σ_{uv∈E} (S_u²+S_v²)^39  (exact u128→u64; S-Pentatetracontyl Sombor α=78)
        //    S-variant generalised Sombor SO^α with α=78: exact integer (no isqrt).
        //    NAMSO(α=76,topo70) → NANSO(α=78,topo71). (3rd-pass AN.)
        //    NANSO = |E|·(2S²)^39 = 549755813888|E|·S^78 for S-regular.
        //    Overflow per edge: (2×16129²)^39 → saturating u128 accumulator.
        //    Implementation: s2s^39 = s2s32 × s2s4 × s2s2 × s2s  (39=32+4+2+1; 4 mults).

        let mut nhpentetraactc_acc: u128 = 0;
        let mut nanso_acc:           u128 = 0;
        for a in 0..nc {
            let sa  = sv[a] as u128;
            let mut bits = adj[a];
            while bits != 0 {
                ...
                if b > a {
                    let sb     = sv[b] as u128;
                    // NHPENTETRAACTC: (S_a + S_b)^44 = ss32 × ss8 × ss4  (44=32+8+4)
                    let ss     = sa + sb;
                    let ss2    = ss * ss;
                    let ss4    = ss2 * ss2;
                    let ss8    = ss4.saturating_mul(ss4);
                    let ss16   = ss8.saturating_mul(ss8);
                    let ss32   = ss16.saturating_mul(ss16);
                    let ss44   = ss32.saturating_mul(ss8).saturating_mul(ss4);
                    nhpentetraactc_acc = nhpentetraactc_acc.saturating_add(ss44);
                    // NANSO: (S_a² + S_b²)^39 = s2s32 × s2s4 × s2s2 × s2s  (39=32+4+2+1)
                    let s2s    = sa * sa + sb * sb;
                    let s2s2   = s2s * s2s;
                    let s2s4   = s2s2.saturating_mul(s2s2);
                    let s2s8   = s2s4.saturating_mul(s2s4);
                    let s2s16  = s2s8.saturating_mul(s2s8);
                    let s2s32  = s2s16.saturating_mul(s2s16);
                    let s2s39  = s2s32.saturating_mul(s2s4).saturating_mul(s2s2).saturating_mul(s2s);
                    nanso_acc = nanso_acc.saturating_add(s2s39);
                }
            }
        }
        let nhpentetraactc = nhpentetraactc_acc.min(u64::MAX as u128) as u64;
        let nanso          = nanso_acc.min(u64::MAX as u128) as u64;
    ...
    pub fn graph_topo_indices72_inner(&self) -> (u64, u64, u64, usize, usize) {
        ...
        // 5. Vertex scan: NHEXTETRAACTC (S-Hexatetracontic vertex sum = Σ_v S(v)^46).
        //
        //    NHEXTETRAACTC(G) = Σ_v S(v)^46  (exact u128→u64; S-Hexatetracontic vertex sum)
        //
        //    Extends: ... → NPENTETRAACTC=Σ S⁴⁵ (topo71) → NHEXTETRAACTC=Σ S⁴⁶ (topo72)
        //    NHEXTETRAACTC = n·S^46 for S-regular.
        //    Overflow: S^46 > u64::MAX for S≥2 → saturating u128 accumulator, clamp to u64::MAX.
        //    Implementation: s^46 = s32 × s8 × s4 × s2  (46=32+8+4+2; 4 mults).

        let mut nhextetraactc_acc: u128 = 0;
        for ci in 0..nc {
            let s   = sv[ci] as u128;
            let s2  = s * s;
            let s4  = s2 * s2;
            ...
            let s16 = s8.saturating_mul(s8);
            let s32 = s16.saturating_mul(s16);
            let s46 = s32.saturating_mul(s8).saturating_mul(s4).saturating_mul(s2);
            nhextetraactc_acc = nhextetraactc_acc.saturating_add(s46);
        }
        let nhextetraactc = nhextetraactc_acc.min(u64::MAX as u128) as u64;

        // 6. Edge scan (a < b): NHHEXTETRAACTC (S-Pentatetracontic edge-sum) and NAOSO (S-Octacontyl Sombor).
        //
        //    NHHEXTETRAACTC(G) = Σ_{uv∈E} (S_u+S_v)^45  (exact u128→u64; S-Pentatetracontic edge-sum)
        //    Extends: → NHPENTETRAACTC=Σ(S+S)⁴⁴ (topo71) → NHHEXTETRAACTC=Σ(S+S)⁴⁵ (topo72)
        //    NHHEXTETRAACTC = |E|·(2S)^45 = 35184372088832|E|·S^45 for S-regular.
        //    Overflow per edge: (2×16129)^45 → saturating u128 accumulator.
        //    Implementation: ss^45 = ss32 × ss8 × ss4 × ss  (45=32+8+4+1; 4 mults).
        //
        //    NAOSO(G) = Σ_{uv∈E} (S_u²+S_v²)^40  (exact u128→u64; S-Octacontyl Sombor α=80)
        //    S-variant generalised Sombor SO^α with α=80: exact integer (no isqrt).
        //    NANSO(α=78,topo71) → NAOSO(α=80,topo72). (3rd-pass AO.)
        //    NAOSO = |E|·(2S²)^40 = 1099511627776|E|·S^80 for S-regular.
        //    Overflow per edge: (2×16129²)^40 → saturating u128 accumulator.
        //    Implementation: s2s^40 = s2s32 × s2s8  (40=32+8; 2 mults — efficient!).

        let mut nhhextetraactc_acc: u128 = 0;
        let mut naoso_acc:           u128 = 0;
        for a in 0..nc {
            let sa  = sv[a] as u128;
            let mut bits = adj[a];
            while bits != 0 {
                ...
                if b > a {
                    let sb     = sv[b] as u128;
                    // NHHEXTETRAACTC: (S_a + S_b)^45 = ss32 × ss8 × ss4 × ss  (45=32+8+4+1)
                    let ss     = sa + sb;
                    let ss2    = ss * ss;
                    let ss4    = ss2 * ss2;
                    let ss8    = ss4.saturating_mul(ss4);
                    let ss16   = ss8.saturating_mul(ss8);
                    let ss32   = ss16.saturating_mul(ss16);
                    let ss45   = ss32.saturating_mul(ss8).saturating_mul(ss4).saturating_mul(ss);
                    nhhextetraactc_acc = nhhextetraactc_acc.saturating_add(ss45);
                    // NAOSO: (S_a² + S_b²)^40 = s2s32 × s2s8  (40=32+8; efficient 2 mults)
                    let s2s    = sa * sa + sb * sb;
                    let s2s2   = s2s * s2s;
                    let s2s4   = s2s2.saturating_mul(s2s2);
                    let s2s8   = s2s4.saturating_mul(s2s4);
                    let s2s16  = s2s8.saturating_mul(s2s8);
                    let s2s32  = s2s16.saturating_mul(s2s16);
                    let s2s40  = s2s32.saturating_mul(s2s8);
                    naoso_acc = naoso_acc.saturating_add(s2s40);
                }
            }
        }
        let nhhextetraactc = nhhextetraactc_acc.min(u64::MAX as u128) as u64;
        let naoso          = naoso_acc.min(u64::MAX as u128) as u64;
    ...
    pub fn graph_topo_indices73_inner(&self) -> (u64, u64, u64, usize, usize) {
        ...
        // 5. Vertex scan: NHEPTETRAACTC (S-Heptatetracontic vertex sum = Σ_v S(v)^47).
        //
        //    NHEPTETRAACTC(G) = Σ_v S(v)^47  (exact u128→u64; S-Heptatetracontic vertex sum)
        //
        //    Extends: ... → NHEXTETRAACTC=Σ S⁴⁶ (topo72) → NHEPTETRAACTC=Σ S⁴⁷ (topo73)
        ...
        let mut nheptetraactc_acc: u128 = 0;
        for ci in 0..nc {
            let s   = sv[ci] as u128;
            let s2  = s * s;
            let s4  = s2 * s2;
            ...
            let s16 = s8.saturating_mul(s8);
            let s32 = s16.saturating_mul(s16);
            let s47 = s32.saturating_mul(s8).saturating_mul(s4).saturating_mul(s2).saturating_mul(s);
            nheptetraactc_acc = nheptetraactc_acc.saturating_add(s47);
        }
        let nheptetraactc = nheptetraactc_acc.min(u64::MAX as u128) as u64;

        // 6. Edge scan (a < b): NHHEPTETRAACTC (S-Hexatetracontic edge-sum) and NAPSO (S-Docosacontyl Sombor).
        //
        //    NHHEPTETRAACTC(G) = Σ_{uv∈E} (S_u+S_v)^46  (exact u128→u64; S-Hexatetracontic edge-sum)
        //    Extends: → NHHEXTETRAACTC=Σ(S+S)⁴⁵ (topo72) → NHHEPTETRAACTC=Σ(S+S)⁴⁶ (topo73)
        //    NHHEPTETRAACTC = |E|·(2S)^46 = 70368744177664|E|·S^46 for S-regular.
        //    Overflow per edge: (2×16129)^46 → saturating u128 accumulator.
        //    Implementation: ss^46 = ss32 × ss8 × ss4 × ss2  (46=32+8+4+2; 4 mults — efficient!).
        //
        //    NAPSO(G) = Σ_{uv∈E} (S_u²+S_v²)^41  (exact u128→u64; S-Docosacontyl Sombor α=82)
        //    S-variant generalised Sombor SO^α with α=82: exact integer (no isqrt).
        //    NAOSO(α=80,topo72) → NAPSO(α=82,topo73). (3rd-pass AP.)
        //    NAPSO = |E|·(2S²)^41 = 2199023255552|E|·S^82 for S-regular.
        //    Overflow per edge: (2×16129²)^41 → saturating u128 accumulator.
        //    Implementation: s2s^41 = s2s32 × s2s8 × s2s  (41=32+8+1; 3 mults).

        let mut nhheptetraactc_acc: u128 = 0;
        let mut napso_acc:           u128 = 0;
        for a in 0..nc {
            let sa  = sv[a] as u128;
            let mut bits = adj[a];
            while bits != 0 {
                ...
                if b > a {
                    let sb     = sv[b] as u128;
                    // NHHEPTETRAACTC: (S_a + S_b)^46 = ss32 × ss8 × ss4 × ss2  (46=32+8+4+2; efficient 4 mults)
                    let ss     = sa + sb;
                    let ss2    = ss * ss;
                    let ss4    = ss2 * ss2;
                    let ss8    = ss4.saturating_mul(ss4);
                    let ss16   = ss8.saturating_mul(ss8);
                    let ss32   = ss16.saturating_mul(ss16);
                    let ss46   = ss32.saturating_mul(ss8).saturating_mul(ss4).saturating_mul(ss2);
                    nhheptetraactc_acc = nhheptetraactc_acc.saturating_add(ss46);
                    // NAPSO: (S_a² + S_b²)^41 = s2s32 × s2s8 × s2s  (41=32+8+1; 3 mults)
                    let s2s    = sa * sa + sb * sb;
                    let s2s2   = s2s * s2s;
                    let s2s4   = s2s2.saturating_mul(s2s2);
                    let s2s8   = s2s4.saturating_mul(s2s4);
                    let s2s16  = s2s8.saturating_mul(s2s8);
                    let s2s32  = s2s16.saturating_mul(s2s16);
                    let s2s41  = s2s32.saturating_mul(s2s8).saturating_mul(s2s);
                    napso_acc = napso_acc.saturating_add(s2s41);
                }
            }
        }
        let nhheptetraactc = nhheptetraactc_acc.min(u64::MAX as u128) as u64;
        let napso          = napso_acc.min(u64::MAX as u128) as u64;
    ...
    pub fn graph_topo_indices75_inner(&self) -> (u64, u64, u64, usize, usize) {
        ...
        // 5. Vertex scan: NNONATETRAACTC (S-Nonatetracontic vertex sum = Σ_v S(v)^49).
        //
        //    NNONATETRAACTC(G) = Σ_v S(v)^49  (exact u128→u64; S-Nonatetracontic vertex sum)
        //
        //    Extends: ... → NOCTOTETRAACTC=Σ S⁴⁸ (topo74) → NNONATETRAACTC=Σ S⁴⁹ (topo75)
        ...
        let mut nnonatetraactc_acc: u128 = 0;
        for ci in 0..nc {
            let s   = sv[ci] as u128;
            let s2  = s * s;
            let s4  = s2 * s2;
            ...
            let s16 = s8.saturating_mul(s8);
            let s32 = s16.saturating_mul(s16);
            let s49 = s32.saturating_mul(s16).saturating_mul(s);
            nnonatetraactc_acc = nnonatetraactc_acc.saturating_add(s49);
        }
        let nnonatetraactc = nnonatetraactc_acc.min(u64::MAX as u128) as u64;

        // 6. Edge scan (a < b): NHNONATETRAACTC (S-Octotetracontic edge-sum) and NARSO (S-Hexaoctacontyl Sombor).
        //
        //    NHNONATETRAACTC(G) = Σ_{uv∈E} (S_u+S_v)^48  (exact u128→u64; S-Octotetracontic edge-sum)
        //    Extends: → NHOCTOTETRAACTC=Σ(S+S)⁴⁷ (topo74) → NHNONATETRAACTC=Σ(S+S)⁴⁸ (topo75)
        ...
        //
        //    NARSO(G) = Σ_{uv∈E} (S_u²+S_v²)^43  (exact u128→u64; S-Hexaoctacontyl Sombor α=86)
        //    S-variant generalised Sombor SO^α with α=86: exact integer (no isqrt).
        //    NAQSO(α=84,topo74) → NARSO(α=86,topo75). (3rd-pass AR.)
        //    NARSO = |E|·(2S²)^43 = 8796093022208|E|·S^86 for S-regular.
        //    Overflow per edge: (2×16129²)^43 → saturating u128 accumulator.
        //    Implementation: s2s^43 = s2s32 × s2s8 × s2s2 × s2s  (43=32+8+2+1; 4 mults).

        let mut nhnonatetraactc_acc: u128 = 0;
        let mut narso_acc:            u128 = 0;
        for a in 0..nc {
            let sa  = sv[a] as u128;
            let mut bits = adj[a];
            while bits != 0 {
                ...
                if b > a {
                    let sb     = sv[b] as u128;
                    // NHNONATETRAACTC: (S_a + S_b)^48 = ss32 × ss16  (48=32+16; 2 mults — very efficient!)
                    let ss     = sa + sb;
                    let ss2    = ss * ss;
                    let ss4    = ss2 * ss2;
                    let ss8    = ss4.saturating_mul(ss4);
                    let ss16   = ss8.saturating_mul(ss8);
                    let ss32   = ss16.saturating_mul(ss16);
                    let ss48   = ss32.saturating_mul(ss16);
                    nhnonatetraactc_acc = nhnonatetraactc_acc.saturating_add(ss48);
                    // NARSO: (S_a² + S_b²)^43 = s2s32 × s2s8 × s2s2 × s2s  (43=32+8+2+1; 4 mults)
                    let s2s    = sa * sa + sb * sb;
                    let s2s2   = s2s * s2s;
                    let s2s4   = s2s2.saturating_mul(s2s2);
                    let s2s8   = s2s4.saturating_mul(s2s4);
                    let s2s16  = s2s8.saturating_mul(s2s8);
                    let s2s32  = s2s16.saturating_mul(s2s16);
                    let s2s43  = s2s32.saturating_mul(s2s8).saturating_mul(s2s2).saturating_mul(s2s);
                    narso_acc = narso_acc.saturating_add(s2s43);
                }
            }
        }
        let nhnonatetraactc = nhnonatetraactc_acc.min(u64::MAX as u128) as u64;
        let narso           = narso_acc.min(u64::MAX as u128) as u64;
    ...
    pub fn graph_topo_indices76_inner(&self) -> (u64, u64, u64, usize, usize) {
        ...
        // 5. Vertex scan: NPENTAACTC (S-Pentacontic vertex sum = Σ_v S(v)^50).
        //
        //    NPENTAACTC(G) = Σ_v S(v)^50  (exact u128→u64; S-Pentacontic vertex sum)
        //
        //    Extends: ... → NNONATETRAACTC=Σ S⁴⁹ (topo75) → NPENTAACTC=Σ S⁵⁰ (topo76)
        ...
        for ci in 0..nc {
            ...
            let s16 = s8.saturating_mul(s8);
            let s32 = s16.saturating_mul(s16);
            let s50 = s32.saturating_mul(s16).saturating_mul(s2);
            npentaactc_acc = npentaactc_acc.saturating_add(s50);
        }
        let npentaactc = npentaactc_acc.min(u64::MAX as u128) as u64;

        // 6. Edge scan (a < b): NHPENTAACTC (S-Nonapentacontic edge-sum) and NASSO (S-Variant Sombor α=88).
        //
        //    NHPENTAACTC(G) = Σ_{uv∈E} (S_u+S_v)^49  (exact u128→u64; S-Nonapentacontic edge-sum)
        //    Extends: → NHNONATETRAACTC=Σ(S+S)⁴⁸ (topo75) → NHPENTAACTC=Σ(S+S)⁴⁹ (topo76)
        ...
        //
        //    NASSO(G) = Σ_{uv∈E} (S_u²+S_v²)^44  (exact u128→u64; S-Variant Sombor α=88)
        //    S-variant generalised Sombor SO^α with α=88: exact integer (no isqrt).
        //    NARSO(α=86,topo75) → NASSO(α=88,topo76). (3rd-pass AS.)
        //    NASSO = |E|·(2S²)^44 = 17592186044416|E|·S^88 for S-regular.
        //    Overflow per edge: (2×16129²)^44 → saturating u128 accumulator.
        //    Implementation: s2s^44 = s2s32 × s2s8 × s2s4  (44=32+8+4; 3 mults — efficient!).

        let mut nhpentaactc_acc: u128 = 0;
        let mut nasso_acc:        u128 = 0;
        for a in 0..nc {
            let sa  = sv[a] as u128;
            let mut bits = adj[a];
            while bits != 0 {
                ...
                if b > a {
                    let sb     = sv[b] as u128;
                    // NHPENTAACTC: (S_a + S_b)^49 = ss32 × ss16 × ss  (49=32+16+1; 3 mults)
                    let ss     = sa + sb;
                    let ss2    = ss * ss;
                    let ss4    = ss2 * ss2;
                    let ss8    = ss4.saturating_mul(ss4);
                    let ss16   = ss8.saturating_mul(ss8);
                    let ss32   = ss16.saturating_mul(ss16);
                    let ss49   = ss32.saturating_mul(ss16).saturating_mul(ss);
                    nhpentaactc_acc = nhpentaactc_acc.saturating_add(ss49);
                    // NASSO: (S_a² + S_b²)^44 = s2s32 × s2s8 × s2s4  (44=32+8+4; 3 mults — efficient!)
                    let s2s    = sa * sa + sb * sb;
                    let s2s2   = s2s * s2s;
                    let s2s4   = s2s2.saturating_mul(s2s2);
                    let s2s8   = s2s4.saturating_mul(s2s4);
                    let s2s16  = s2s8.saturating_mul(s2s8);
                    let s2s32  = s2s16.saturating_mul(s2s16);
                    let s2s44  = s2s32.saturating_mul(s2s8).saturating_mul(s2s4);
                    nasso_acc = nasso_acc.saturating_add(s2s44);
                }
            }
        }
        let nhpentaactc = nhpentaactc_acc.min(u64::MAX as u128) as u64;
        let nasso       = nasso_acc.min(u64::MAX as u128) as u64;
    ...
    pub fn graph_topo_indices77_inner(&self) -> (u64, u64, u64, usize, usize) {
        ...
        // 5. Vertex scan: NHENPENTAACTC (S-Henpentacontic vertex sum = Σ_v S(v)^51).
        //
        //    NHENPENTAACTC(G) = Σ_v S(v)^51  (exact u128→u64; S-Henpentacontic vertex sum)
        //
        //    Extends: → NPENTAACTC=Σ S⁵⁰ (topo76) → NHENPENTAACTC=Σ S⁵¹ (topo77)
        ...
        let mut nhenpentaactc_acc: u128 = 0;
        for ci in 0..nc {
            let s   = sv[ci] as u128;
            let s2  = s * s;
            let s4  = s2 * s2;
            ...
            let s16 = s8.saturating_mul(s8);
            let s32 = s16.saturating_mul(s16);
            let s51 = s32.saturating_mul(s16).saturating_mul(s2).saturating_mul(s);
            nhenpentaactc_acc = nhenpentaactc_acc.saturating_add(s51);
        }
        let nhenpentaactc = nhenpentaactc_acc.min(u64::MAX as u128) as u64;

        // 6. Edge scan (a < b): NHHENPENTAACTC (S-Pentacontic edge-sum) and NATSO (S-Variant Sombor α=90).
        //
        //    NHHENPENTAACTC(G) = Σ_{uv∈E} (S_u+S_v)^50  (exact u128→u64; S-Pentacontic edge-sum)
        //    Extends: → NHPENTAACTC=Σ(S+S)⁴⁹ (topo76) → NHHENPENTAACTC=Σ(S+S)⁵⁰ (topo77)
        ...
        //
        //    NATSO(G) = Σ_{uv∈E} (S_u²+S_v²)^45  (exact u128→u64; S-Variant Sombor α=90)
        //    S-variant generalised Sombor SO^α with α=90: exact integer (no isqrt).
        //    NASSO(α=88,topo76) → NATSO(α=90,topo77). (3rd-pass AT.)
        //    NATSO = |E|·(2S²)^45 = 35184372088832|E|·S^90 for S-regular.
        //    Overflow per edge: (2×16129²)^45 → saturating u128 accumulator.
        //    Implementation: s2s^45 = s2s32 × s2s8 × s2s4 × s2s  (45=32+8+4+1; 4 mults).

        let mut nhhenpentaactc_acc: u128 = 0;
        let mut natso_acc:           u128 = 0;
        for a in 0..nc {
            let sa  = sv[a] as u128;
            let mut bits = adj[a];
            while bits != 0 {
                ...
                if b > a {
                    let sb     = sv[b] as u128;
                    // NHHENPENTAACTC: (S_a + S_b)^50 = ss32 × ss16 × ss2  (50=32+16+2; 3 mults — efficient!)
                    let ss     = sa + sb;
                    let ss2    = ss * ss;
                    let ss4    = ss2 * ss2;
                    let ss8    = ss4.saturating_mul(ss4);
                    let ss16   = ss8.saturating_mul(ss8);
                    let ss32   = ss16.saturating_mul(ss16);
                    let ss50   = ss32.saturating_mul(ss16).saturating_mul(ss2);
                    nhhenpentaactc_acc = nhhenpentaactc_acc.saturating_add(ss50);
                    // NATSO: (S_a² + S_b²)^45 = s2s32 × s2s8 × s2s4 × s2s  (45=32+8+4+1; 4 mults)
                    let s2s    = sa * sa + sb * sb;
                    let s2s2   = s2s * s2s;
                    let s2s4   = s2s2.saturating_mul(s2s2);
                    let s2s8   = s2s4.saturating_mul(s2s4);
                    let s2s16  = s2s8.saturating_mul(s2s8);
                    let s2s32  = s2s16.saturating_mul(s2s16);
                    let s2s45  = s2s32.saturating_mul(s2s8).saturating_mul(s2s4).saturating_mul(s2s);
                    natso_acc = natso_acc.saturating_add(s2s45);
                }
            }
        }
        let nhhenpentaactc = nhhenpentaactc_acc.min(u64::MAX as u128) as u64;
        let natso          = natso_acc.min(u64::MAX as u128) as u64;
