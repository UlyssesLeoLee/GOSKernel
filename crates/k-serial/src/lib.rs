...
unsafe extern "C" fn serial_on_event(
    ...
    // ── Pre-processing: extract the data byte from the signal ─────────────────
    let Some(input) = pre::prepare(event) else {
        return ExecStatus::Done;
    };
    // ── Main processing: write byte to UART ───────────────────────────────────
    let Some(output) = proc::process(input) else {
        return ExecStatus::Done;
    };
    // ── Post-processing: update telemetry state ───────────────────────────────
